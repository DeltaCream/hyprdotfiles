...
var (
	Name             = "clipvault"
	NamePretty       = "Clipvault"
	file             = common.CacheFile("clipvault.gob")
...
func Available() bool {
	p, err := exec.LookPath("clipvault")
	if p == "" || err != nil {
		slog.Info(Name, "available", "clipvault not found. disabling")
		return false
	}
...
const clipvaultPollInterval = 2 * time.Second

func pollClipvault() {
	// initial load
	loadClipvaultListOnce()

	// then poll
	ticker := time.NewTicker(clipvaultPollInterval)
	defer ticker.Stop()
...
func loadClipvaultListOnce() {
	// clipvault list prints tab-separated lines; first column is index (relative),
	// second column (preview) is what pickers use. We'll iterate the list and
	// fetch the actual bytes with `clipvault get --index N`.
	cmd := exec.Command("clipvault", "list")
	out, err := cmd.CombinedOutput()
	if err != nil {
		slog.Debug(Name, "clipvault list", err, "msg", string(out))
		return
	}

	// build new map from clipvault contents
	newHistory := make(map[string]*Item)
	...
	for scanner.Scan() {
		...
		// clipvault list lines are tab-separated: "<index>\t<preview...>"
		...
		// fetch raw bytes for this relative index
		getCmd := exec.Command("clipvault", "get", "--index", indexStr)
		data, getErr := getCmd.CombinedOutput()
		if getErr != nil {
			// debug but continue with other items
			slog.Debug(Name, "clipvault get", getErr, "index", indexStr, "msg", string(data))
			continue
		}

		// compute md5 key (same approach as current code)
		sum := md5.Sum(data)
		key := hex.EncodeToString(sum[:])

		// if we already have in existing cache, reuse (so timestamps stay correct)
		mu.Lock()
		if existing, ok := clipboardhistory[key]; ok {
			// preserve existing item (update time)
			existing.Time = time.Now()
			newHistory[key] = existing
			mu.Unlock()
			continue
		}
		mu.Unlock()

		// decide if this is text or binary (image)
		if utf8.Valid(data) {
			// text
			txt := string(data)

			// respect IgnoreSymbols filter
			if config.IgnoreSymbols {
				if _, ok := symbols[txt]; ok {
					continue
				}
			}
			...
			// try identify to get format
			identify := exec.Command("identify", "-format", "%m", "-")
			identify.Stdin = bytes.NewReader(data)
			idOut, idErr := identify.CombinedOutput()
			if idErr != nil {
				slog.Debug(Name, "identify", idErr, "msg", string(idOut))
				// fallback: save raw bytes with a generic ext
				filePath := saveImg(data, "bin")
				if filePath != "" {
					newHistory[key] = &Item{
						Img:   filePath,
						Time:  time.Now(),
			...
			ext := strings.ToLower(strings.TrimSpace(string(idOut)))
			filePath := saveImg(data, ext)
			if filePath != "" {
				newHistory[key] = &Item{
					Img:   filePath,
					Time:  time.Now(),
	...
	// merge: keep entries from previous clipboardhistory that still exist (so timestamp ordering preserved)
	mu.Lock()
	for k, v := range clipboardhistory {
		if _, ok := newHistory[k]; ok {
			// newHistory already contains updated item; keep its value
			continue
		}
		// Optionally preserve old items that clipvault no longer lists?
		// I choose not to; if you want to persist old entries, uncomment:
		newHistory[k] = v
	}

	// replace global history and persist
